    ...
    <>
      <Box>
        ...
        <Box sx={{ mb: "1rem", width: "100%", px: "1rem", display: "flex", justifyContent: "center", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
          {_DATA.currentData().map((product: { id: number; title: string; image: string; price: number; amount: number; balance: number }) => {
            return <Cards amount={product.amount} id={product.id} key={product.id} title={product.title} image={product.image} price={product.price} balance={product.balance} />;
          })}
        </Box>
